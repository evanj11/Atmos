...
function loadPdbThen(frameIdx: number, applyFn: (c: NGLComponent) => void): void {
  invoke<string>('get_snapshot_pdb', { frame_idx: frameIdx })
    .then(pdb => {
...
function loadUmbrellaThen(windowIdx: number, applyFn: (c: NGLComponent) => void): void {
  invoke<string>('get_umbrella_snapshot_pdb', { window_idx: windowIdx })
    .then(pdb => {
      const blob = new Blob([pdb], { type: 'text/plain' });
      return stage!.loadFile(blob, { ext: 'pdb', firstModelOnly: true });
    })
...
function applyResidueReps(atomIndex: number) {
  ...
  if (bgIndices.length > 0) {
    comp.addRepresentation('backbone', { sele: bgSel, colorScheme: currentColor, opacity: 0.50, radius: 0.1 });
  }
...
function applyPairReps(iAtom: number, jAtom: number) {
  ...
  if (bgIndices.length > 0) {
    comp.addRepresentation('backbone', { sele: bgSel, colorScheme: 'uniform', color: '#4a7fa5', opacity: 1.0, radius: 0.15 });
  }
...
function modeFrame(frameIdx: number) {
  ...
  loadPdbThen(0, async () => {
    await fetchSelectionResidues();
    const flat = await invoke<number[]>('get_frame_coords', { frame_idx: frameIdx });
...
// ─── Dihedral mode ────────────────────────────────────────────────────────────
//
// Highlights the selected residue in 3D and renders a floating mini
// Ramachandran canvas overlay of that residue's φ/ψ trajectory.
...
function modeDihedral(atomIndex: number) {
  ...
  loadPdbThen(0, async () => {
    ...
    let dihedData: any = null;
    try {
      dihedData = await invoke<any>('get_residue_dihedrals', { atom_idx: atomIndex });
    } catch (_) {
      setInfoPill(`${label} — run Ramachandran analysis first for dihedral overlay`);
      return;
    }

    const phi = dihedData.phi as number[], psi = dihedData.psi as number[];
    ...
    document.getElementById('dihedral-overlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'dihedral-overlay';
    overlay.style.cssText = `position:absolute;top:60px;right:16px;z-index:20;
      background:rgba(13,14,15,0.88);border:1px solid #2e3235;border-radius:5px;
      padding:10px;display:flex;flex-direction:column;gap:6px;backdrop-filter:blur(4px);`;
    document.querySelector('.middle')?.appendChild(overlay);

    const SIZE = 200, PAD = 20, inner = SIZE - PAD * 2;
    ...
    const toC = (phi: number, psi: number): [number,number] => [
    ...
    const [x0] = toC(0, -180), [,y0] = toC(-180, 0);
    ...
    // α and β region hints
    ctx.fillStyle = 'rgba(0,196,167,0.07)'; ctx.strokeStyle = 'rgba(0,196,167,0.15)'; ctx.lineWidth = 0.5;
    ctx.beginPath();
    [[-145,-60],[-30,-60],[-30,20],[-145,20],[-145,-60]].forEach(([p,s],i) => {
      const [cx,cy] = toC(p,s); i===0 ? ctx.moveTo(cx,cy) : ctx.lineTo(cx,cy);
    }); ctx.closePath(); ctx.fill(); ctx.stroke();

    ctx.fillStyle = 'rgba(91,141,238,0.07)'; ctx.strokeStyle = 'rgba(91,141,238,0.15)';
    ctx.beginPath();
    [[-170,90],[-55,90],[-55,180],[-170,180],[-170,90]].forEach(([p,s],i) => {
      const [cx,cy] = toC(p,s); i===0 ? ctx.moveTo(cx,cy) : ctx.lineTo(cx,cy);
    }); ctx.closePath(); ctx.fill(); ctx.stroke();

    // Scatter points coloured cold→warm by frame time
    ...
    for (const [ph, ps, t] of validPairs) {
      ...
      const r = Math.round(50 + frac*205), g = Math.round(100 - frac*60), b = Math.round(220 - frac*180);
    ...
    // Mean position
    const phiF = phi.filter(v => isFinite(v)), psiF = psi.filter(v => isFinite(v));
    if (phiF.length > 0) {
      const phiMean = phiF.reduce((a,b) => a+b, 0) / phiF.length;
      const psiMean = psiF.reduce((a,b) => a+b, 0) / psiF.length;
      const [mx, my] = toC(phiMean, psiMean);
      ctx.fillStyle = '#fff'; ctx.strokeStyle = '#0d0e0f'; ctx.lineWidth = 0.5;
      ctx.beginPath(); ctx.arc(mx, my, 4, 0, Math.PI*2); ctx.fill(); ctx.stroke();

      ctx.fillStyle = '#7a7f85'; ctx.font = `${Math.round(8*dpr)/dpr}px monospace`;
      ...
      stats.style.cssText = 'font-family:var(--font-mono);font-size:9px;color:var(--muted);line-height:1.5;';
      stats.innerHTML = `φ̄ = ${phiMean.toFixed(1)}°<br>ψ̄ = ${psiMean.toFixed(1)}°<br>${validPairs.length} valid frames`;
...
async function init() {
  ...
  el('style-group').addEventListener('click', (e) => {
    ...
    switch (mode) {
      ...
      case 'frame':    applyFrameReps();                break;
      case 'residue':
